...
export const validationMantenimiento = Yup.object().shape({
  ...
  horometro: Yup.number().required('El horometro es requerido'),
  descripcion: Yup.string().required('La descripcion es requerida'),
  modalidad: Yup.string().required('La modalidad es requerida'),
  costo: Yup.number().required('El valor es requerido'),
  soporte: Yup.mixed().when('edit', (edit) => {
    if (!edit) {
      return Yup.mixed().required('El soporte de la factura es requerido');
    }
  })
});
